...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top" id="mainNav">
 ...
 <?php }
 if(isLoggedIn() &&  $_SESSION['type'] == 'employer'){
 ...
 ?>
  ...
  <?php 
   global $conn;
   $sql = "SELECT * from ordertable where EmployeID=$userID and Accepted=0;"; 
   $result = $conn->query($sql);
   $rowcount = mysqli_num_rows( $result );
 echo"<br> ".$rowcount. ""; ?>
   <br><br>
   ...
   <a href="CommentsNotfications.php" class="notification">
 <i class="fa fa-bell"></i> 
  <span class="badge"> 
   <?php 
   global $conn;
   $sql = "SELECT * from commenttable where EmployeUid=$userID and ViewdByEmploye=0;"; 
   $result = $conn->query($sql);
   $rowcount = mysqli_num_rows( $result );
 echo"<br> ".$rowcount. ""; ?>
      <br><br>
 ...
 <a></a> <a class="navbar-brand" href="index.php"><img src="assets/img/Logo4.svg" alt="..." /></a>               
  ...
  <?php if(isLoggedIn() && ($_SESSION['type'] == 'employer' || $_SESSION['type'] == 'admin')){     
  echo '<li class="nav-item"><a class="nav-link" href="AddNewProductEmployeuser.php"> منتج جديد</a></li>';
  } ?>   
                       ...
                       <?php if(isLoggedIn()){
                           ...
                           echo'<li class="nav-item "><a href="ProfilePage.php?useruid='.$_SESSION["uid"].'" class="nav-link">Welcome '.$_SESSION['fname'].'</a></li>';
                           } ?>
      	               <?php if(!isLoggedIn())
                       {
 			echo '<li class="nav-item"><a class="nav-link" href="login.php">تسجيل الدخول</a></li> ';
 			echo '<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false"> التسجيل </a>                
		        <ul name ="typeOfUser" l class="dropdown-menu" aria-labelledby="navbarDropdown">
                        <li><a class="dropdown-item" href="Signupform.php">مستخدم عادي</a></li>               
	                <li><a class="dropdown-item"href="Signup.php">مستخدم لديه مهنة </a></li>  
		        </ul>
                        </li> ';
		      } ?>                
                    </ul>                     
